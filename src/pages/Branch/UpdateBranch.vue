...
<div class="box-title box-title-fix">
    <h2 class="title-page">Update Branch</h2>
    <div class="btn-group float-right">
        <form class="buttonLogout"  @submit.prevent="handleUpdate"  >
        <router-link to="/management/branch">
        <b-button  
        id="show-btn" 
...
<div class="card form-fix">
    <div class="row">
        <div class="col-sm-12">
            <div class="card-body">
                <h4 class="card-title mb-3" style="font-size: 15px;">Information Branch</h4>
...
components: { index },
    ...
    methods:{
            async handleUpdate(){
        try{
            const response = await BranchService.update(this.token, this.dataBranch, this.idBranch)
    ...
    async fetchData(){
            const response = await BranchService.getDetail(this.token ,this.idBranch) 
            if (response.status == 200) {
