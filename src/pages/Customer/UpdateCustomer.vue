...
<div class="box-title box-title-fix">
    <h2 class="title-page">Update Customer</h2>
...
<div class="card form-fix">
    <div class="row">
        <div class="col-sm-12">
            <div class="card-body">
                <h4 class="card-title mb-3" style="font-size: 15px;">Information Customer</h4>
...
components: { index },
    ...
    methods:{
            async handleUpdate(){
        try{
            const response = await CustomerService.update(this.token, this.dataCustomer, this.idCustomer)
    ...
    async fetchData(){
            const response = await CustomerService.getDetail(this.token ,this.idCustomer) 
            if (response.status == 200) {
