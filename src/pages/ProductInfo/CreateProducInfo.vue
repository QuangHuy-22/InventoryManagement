...
<!-- InstanceBeginEditable name="EditRegion1" -->
    ...
    <div class="box-title box-title-fix">
        ...
        <form class="buttonAddUser" @submit.prevent="handleAddProducInfo"  >
...
<form class="col-md-6">
        <h1 class="form-inner-heading">Ảnh đính kèm</h1>
...
axios({
    ...
    data: {
    name: this.dataProducInfo.name,
    description: this.dataProducInfo.description,
    imgUrl: this.someData,
    categoryId: this.dataProducInfo.categoryId
},
}).then((response)=>{
  ...
  onSelectFile () {
      ...
      this.formData.append('folderName', 'NextCrm')
      this.arrayFormData.push(this.formData)
      if (files && files[0]) {
    ...
    removeImg (index) {
      ...
      this.arrayFormData.forEach((item, i) => {
        if (index === i) {
          this.arrayFormData.splice(i, item)
        }
      })
    },
    },
